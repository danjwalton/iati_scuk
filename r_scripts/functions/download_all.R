...
new_download <- function(name = "fcdo_all", reporting_ref = "GB-GOV-1", date_from = NULL, date_to = NULL, policy_markers = T, extra_info = T, sector_fill = T, fcdo_portfolio = T){
  ...
  download <- readRDS(dportal_pull(reporting_ref = reporting_ref, date_from = date_from, date_to = date_to))
  
  download <- prettify(download, policy_markers = policy_markers, extra_info = extra_info)
  ...
  output2 <- paste0("rdatas/", name, "_[", Sys.Date(), "].csv")
